...
**Instance IDs (per-creature identity).** Every `Creature` carries a stable `id` — a short random string (`"c_" + …`, via `newCreatureId()` in `engine/creature.ts`) generated **once at creation** (starter, quest reward, Guardian-signature/Champion trophy, Hunt tame) and **never changed**. Unlike `speciesId` — which is shared by every pony of a species and drives art, stat computation, element, and display name — `id` distinguishes **individuals**. Today two ponies can't share a species, so the roster and active team were historically keyed by `speciesId`; the instance `id` future-proofs that for **breeding** and **New Game+** (§15), where two creatures of the same species can coexist. The **active team** stores instance ids (not speciesIds), and `resolveBattleTeam` / the picker resolve members by `Creature.id`, so telling apart two same-species ponies is unambiguous. `id` is optional on the type only for back-compat: the **save migration backfills** a fresh id for any pre-id creature and **remaps** a legacy speciesId-based `activeTeam` to the matching creatures' ids — idempotent, so re-running is safe. *(Analogous to IVs: rolled once on acquisition, permanent, individual.)*
...
- **M3a — Puzzle system refactor ✅ DONE:** three problem categories (Math/Logic/Comprehension), reinforcement weighting, Hunt puzzle phase, flat XP curve. Key changes:
  ...
  - **Cleanup — Creature instance IDs + dead HP display ✅ DONE:** two roster hygiene fixes.
